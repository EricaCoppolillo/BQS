...
class DataLoader:
    ...
    def iter_test_ensemble(self, model_a, model_b, batch_size=256, tag='test', cutoff=5):
        ...
        for batch_idx, (x, pos, neg, mask, pos_te, neg_te, mask_te) in enumerate(self.iter_test(batch_size=batch_size, tag=tag)):
            ...
            y_a, _, _ = model_a(x_input, True)
            y_b, _, _ = model_b(x_input, True)
...
def evaluate(model, dataloader, normalized_popularity, tag='validation'):
...
with open(baseline_file_model, 'rb') as f:
    baseline_model = torch.load(f)

with open(popularity_file_model, 'rb') as f:
    popularity_model = torch.load(f)

baseline_model.eval()
popularity_model.eval()

baseline_result_test = evaluate(baseline_model, trainloader, popularity, 'test')
popularity_result_test = evaluate(popularity_model, trainloader, popularity, 'test')

print(f'K = {settings.gamma_k}')
print('BASELINE MODEL -------------------------------------')
print('\n'.join([f'{k:<23}{v}' for k, v in sorted(baseline_result_test.items())]))

print('POPULARITY MODEL -----------------------------------')
print('\n'.join([f'{k:<23}{v}' for k, v in sorted(popularity_result_test.items())]))
