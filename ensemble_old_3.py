...
class EnsembleMultiVAE(nn.Module):
    ...
    def __init__(self, data_dir, n_items):
        ...
        for param in self.popularity_model.parameters():
            # param.requires_grad = False
            pass

        self.dummy_variable = torch.zeros(n_items, requires_grad=True)

        print('ensemble models loaded!')

    def forward(self, x, predict=True):

        # return self.baseline_model(x.detach()).detach()
        self.dummy_variable = x

        # with torch.no_grad():
        y, mu, logvar = self.baseline_model(x)
...
model = EnsembleMultiVAE(data_dir, n_items)
