...
for seed in "${seeds[@]}"; do
  ...
  for dataset_name in "${datasets[@]}"; do
    ...
    # soup model
    for soup_weight in $(seq 0.0 0.05 0.45); do
      soup_weight="${soup_weight//,/.}"
      jsonStr=$(cat $bpr_config_file)
      jq '.model_type = "model_types.SOUP"' <<<"$jsonStr" > $bpr_config_file
      jsonStr=$(cat $bpr_config_file)
      jq '.soup_weight = "'$soup_weight'"' <<<"$jsonStr" > $bpr_config_file
      jsonStr=$(cat $bpr_config_file)
      jq '.regularizer = ""' <<<"$jsonStr" > $bpr_config_file
      python bpr_soup.py $bpr_config_file
    done
    # oversampling
    jsonStr=$(cat $bpr_config_file)
