...
for seed in "${seeds[@]}"; do
  for dataset_name in "${datasets[@]}"; do
    ...
    # model soup
    for soup_weight in $(seq 0.0 0.05 0.8); do
      soup_weight="${soup_weight//,/.}"
      jsonStr=$(cat rvae_config.json)
      jq '.model_type = "model_types.SOUP"' <<<"$jsonStr" > rvae_config.json
      jsonStr=$(cat rvae_config.json)
      jq '.soup_weight = "'$soup_weight'"' <<<"$jsonStr" > rvae_config.json
      jsonStr=$(cat rvae_config.json)
      jq '.regularizer = ""' <<<"$jsonStr" > rvae_config.json
      python rvae_soup.py ${seed2baseline[$seed]} ${seed2oversampling[$seed]} ${seed2low[$seed]}
    done
    # oversampling
    jsonStr=$(cat rvae_config.json)
