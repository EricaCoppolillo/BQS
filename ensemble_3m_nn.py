...
class EnsembleMultiVAE(nn.Module):
    ...
    def __init__(self, n_items):
        ...
        self.bn_a_0 = nn.BatchNorm1d(n_items)
        self.bn_b_0 = nn.BatchNorm1d(n_items)
        self.bn_c_0 = nn.BatchNorm1d(n_items)

        self.linear_a_0 = nn.Linear(n_items * 3, n_items)
        self.linear_b_0 = nn.Linear(n_items * 3, n_items)
        self.linear_c_0 = nn.Linear(n_items * 3, n_items)

        self.linear_a_1 = nn.Linear(n_items, n_items)
        self.linear_b_1 = nn.Linear(n_items, n_items)
        self.linear_c_1 = nn.Linear(n_items, n_items)

        self.linear_a_2 = nn.Linear(n_items, n_items)
        self.linear_b_2 = nn.Linear(n_items, n_items)
        self.linear_c_2 = nn.Linear(n_items, n_items)

        self.linear_e_0 = nn.Linear(n_items * 4, n_items)
    ...
    def forward(self, x, popularity, y_a, y_b, y_c, predict=False):
        ...
        z_a = self.normalize(y_a)
        z_b = self.normalize(y_b)
        z_c = self.normalize(y_c)
        '''
        z_a = torch.cat((x, p, z_a), 1)
        z_b = torch.cat((x, p, z_b), 1)
        z_c = torch.cat((x, p, z_c), 1)

        z_a = self.linear_a_0(z_a)
        z_b = self.linear_b_0(z_b)
        z_c = self.linear_c_0(z_c)
        ...
        z_e = torch.cat((p, z_a, z_b, z_c), 1)

        z_e = self.linear_e_0(z_e)

        return y_c

    def forward_old(self, x, popularity, y_a, y_b, y_c, predict=False):
        ...
        n_a = self.linear_a_0(torch.cat((x, p, n_a), 1))
        n_b = self.linear_b_0(torch.cat((x, p, n_b), 1))
        n_c = self.linear_c_0(torch.cat((x, p, n_c), 1))
        ...
        y_e = self.linear_e_0(torch.cat((n_a, n_b, n_c), 1))
        # n_a = self.d_e(y_e)
