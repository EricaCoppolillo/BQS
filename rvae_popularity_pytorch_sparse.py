...
class DataLoader:
    def __init__(self, file_tr, pos_neg_ratio=4, negatives_in_test=100, use_popularity=False, max_popularity_position=100):
        ...
        self.max_popularity = self.sorted_item_popularity[-max_popularity_position]
        ...
        print('self.double_frequencies[:20]:', self.double_frequencies[:20])
        ...
        # print('num(max_frequency):', sum(self.frequencies == max(self.frequencies)))
        # print('num(min_frequency):', sum(self.frequencies == min(self.frequencies)))
        print('sorted(self.sorted_item_popularity)[:100]:', sorted(self.sorted_item_popularity[:100]))
        print('sorted(self.sorted_item_popularity)[-100:]:', sorted(self.sorted_item_popularity[-100:]))
        print('sorted(frequencies):', sorted(self.frequencies)[:100])
    ...
    def _generate_mask_tr(self, tag):
        ...
        # IMPROVED VERSION
        # self.mask[tag] = csr_matrix((self.size[tag], self.max_width))
        # pos_temp = csr_matrix((self.size[tag], self.max_width))
        # neg_temp = csr_matrix((self.size[tag], self.max_width))
        ...
        for row in range(self.size[tag]):
            ...
            coo_list[-1:-1] = [[row, col] for col in range(len(self.pos[tag][row]))]
            pos_list[-1:-1] = self.pos[tag][row]
            neg_list[-1:-1] = self.neg[tag][row]
            # length += len(self.pos[tag][row])
        ...
        print('-----------------------------')
        # print('lenght:', length)
        print('len(coo_list):', len(coo_list))
        print('coo_list[:10]:', coo_list[:10])
        print('-----------------------------')

        i = torch.LongTensor(coo_list)
        print('i done!')
        v_mask = torch.IntTensor([1] * len(coo_list))
        print('v_mask done!')
        v_pos = torch.LongTensor(pos_list)  # reduce(operator.concat, self.pos[tag]))
        print('v_pos done!')
        v_neg = torch.LongTensor(neg_list)  # reduce(operator.concat, self.neg[tag]))
        print('v_neg done!')

        print('Creating mask, pos, neg...')
        self.mask[tag] = torch.sparse.IntTensor(i.t(), v_mask, torch.Size([self.size[tag], self.max_width]))
        self.pos[tag] = torch.sparse.LongTensor(i.t(), v_pos, torch.Size([self.size[tag], self.max_width]))
        self.neg[tag] = torch.sparse.LongTensor(i.t(), v_neg, torch.Size([self.size[tag], self.max_width]))
        print('Done!')
