...
class DataLoader:
    def __init__(self, file_tr, pos_neg_ratio=4, negatives_in_test=100, pos_pos_pairs=False):
        ...
        low_pop = len([i for i in self.item_popularity if i <= self.thresholds[0]])
        med_pop = len([i for i in self.item_popularity if self.thresholds[0] < i <= self.thresholds[1]])
        high_pop = len([i for i in self.item_popularity if self.thresholds[1] < i])

        print('type(self.item_popularity):', type(self.item_popularity))
        print('low_pop:', low_pop)
        print('med_pop:', med_pop)
        print('high_pop:', high_pop)
    ...
    def _generate_mask_tr(self, tag):
        ...
        # print('tag:',tag)
        # print('self.neg[tag]:',self.neg[tag])
        # print('self.size[tag]:',self.size[tag])

        width = -1
        for i in range(self.size[tag]):
            x1 = self.pos[tag][i]
            x2 = self.neg[tag][i]
            width = max(width, len(x1), len(x2))
        for i in range(self.size[tag]):
            x1 = self.pos[tag][i]
            x2 = self.neg[tag][i]
        ...
        self.pos[tag] = np.array(self.pos[tag], dtype=np.int32).reshape(self.size[tag], width)

        self.neg[tag] = np.array(self.neg[tag], dtype=np.int32).reshape(self.size[tag], width)

        self.mask[tag] = self.pos[tag] >= 0
        self.pos[tag][self.pos[tag] < 0] = 0
        self.neg[tag][self.neg[tag] < 0] = 0
    ...
    def _generate_training_pairs(self, pos):
        if not self.pos_pos_pairs or len(pos) == 1:
            return self._generate_pairs(pos)
        else:
            positives_a = pos * self.pos_neg_ratio
            negatives_a = self._sample_negatives(pos, len(positives_a))

            # items in pos are id 0-based
            ordered_pos = sorted(pos, key=lambda item: self.item_popularity[item])
            positives_b = ordered_pos[:-1]
            negatives_b = ordered_pos[1:]

            positives = positives_a + positives_b
            negatives = negatives_a + negatives_b

            positives = positives_b[:3] * 20
            negatives = self._sample_negatives(pos, 60)

            '''
            if len(positives)<10:
                print('LC > Pos:', pos)
                print('LC > positives_b:', positives_b)
                print('LC > negatives_b:', negatives_b)
                print('LC > Positives:', positives)
                print('LC > Negatives:', negatives)
                print('LC > len(Positives):', len(positives))
                print('LC > len(Negatives):', len(negatives))
            '''
...
"""x = load_dataset(dataset_file)

validation_data = x['validation_data']

for user_id in validation_data:
    positives_tr, positives_te, negatives_sampled = validation_data[user_id]
    if len(positives_tr) == 0: print('positives tr empty', user_id)
    if len(positives_te) == 0: print('positives tE empty', user_id)

trainloader = DataLoader(dataset_file, pos_neg_ratio=1)

for tag in 'validation test'.split():
    print(tag, 'get_items_counts_by_cat =', trainloader.get_items_counts_by_cat(tag))
    print(tag, 'get_users_counts_by_cat =', trainloader.get_users_counts_by_cat(tag))
    
for tag in 'train validation test'.split():
    for x, pos, neg, mask in trainloader.iter(tag=tag):
        print(tag)
        print(x.shape, pos.shape, neg.shape, mask.shape)
        break

for tag in 'validation test'.split():
    for x, pos, neg, mask, pos_te, neg_te, mask_pos_te in trainloader.iter_test(tag=tag):
        print(tag)
        print(x.shape, pos.shape, neg.shape, mask.shape)
        print(len(pos_te), len(neg_te), len(mask_pos_te))
        break
...
"""# compute_metric(x, y, pos, neg, popularity, popularity_thresholds, top_k=10)

pos = [[0,1,2]]
neg = [[3,4,5]]
x = np.zeros((1, 6))
y = np.array([[.5,.5,.5,.1,.1,.1]])
popularity = [.1,.5,.9,.1,.5,.9]
popularity_thresholds = (.3, .7)

print(compute_metric(x, y, pos, neg, popularity, popularity_thresholds, top_k=1))


pos = [[0,1,2]]
neg = [[3,4,5]]
x = np.zeros((1, 6))
y = np.array([[.1,.1,.5,.1,.1,.1]])
popularity = [.1,.5,.9,.1,.5,.9]
popularity_thresholds = (.3, .7)
...
$$
    log \ p (y|x_u) = - \sum_{i:y_i=1} \sum_{j:y_j=0} \log \sigma (y_j - y_i)
$$

**Loss v2**

$$
    log \ p (y|x_u) = - \sum_{i:y_i=1} \sum_{j:y_j=0} \log \sigma (\gamma_i \cdot (y_j - y_i))
$$
$$
    \gamma_i = \frac {k} {1 + popularity(i^+)}
$$
"""
...
class rvae_rank_pair_loss(rvae_loss):
    ...
    def log_p(self, x, y, pos_items, neg_items, mask):
        ...
        # neg_ll = - torch.sum(torch.pow(freq_pos, 1) * self.logsigmoid((y1 - y2) * weight)) / mask.sum()
        # neg_ll = - torch.sum(torch.pow(freq_pos, 1) * self.logsigmoid((y1 - y2) * weight) + self.logsigmoid((y1 - y2) * weight)) / mask.sum()
        # neg_ll = - torch.sum(torch.pow(freq_pos, 1.5) * self.logsigmoid((y1 - y2) * weight) + self.logsigmoid((y1 - y2) * weight)) / mask.sum()
        # neg_ll = - torch.sum(torch.pow(freq_pos, 1) * self.logsigmoid((y1 - y2) * weight)) / mask.sum()
        # neg_ll = - torch.sum(self.logsigmoid(y1 * freq_pos.float() - y2 * freq_neg.float()) * weight) / mask.sum()

        # neg_ll = - torch.sum(freq_pos * self.logsigmoid((y1 - y2) * weight)) / mask.sum()
        neg_ll = - torch.sum(freq_pos * self.logsigmoid((y1 - y2) * weight)) # / mask.sum()
...
trainloader = DataLoader(dataset_file, pos_pos_pairs=False)
...
# settings.p_dims = [100, 200, 600, n_items]
...
# settings.learning_rate = 3e-4  # 1e-3  # 1e-5
settings.learning_rate = 1e-3  # 1e-3  # 1e-5
settings.optim = 'adam'
settings.scale = 1000
settings.use_popularity = True
# settings.p_dims = [5, 10, 50, 100, 200, 600, n_items]
