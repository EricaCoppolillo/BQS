...
dataset_str = CITEULIKE
loss_type = BASELINE_LOSS  # IMPROVED_LOSS
limit = 0.1  # %
...
if TRAIN:
    ...
    k, k_trainable, \
    a, b, \
    neg_items = get_train_parameters(dataset_str, loss_type)
    # ------------------------------------------
    ...
    for learning_rate in learning_rates:
        for batch_size in batch_sizes:
            ...
            def fit(learning_rate, batch_size, model_path):
                validation_loss = None
                return validation_loss


            fit(learning_rate, batch_size)
            print('INFO:')
...
if TEST:
    ...
    s = ""
    scores, items, out, loss = get_model_scores(session,
                                                dataset.test_data,
                                                dataset.item_users_list,
                                                model.input_users,
                                                model.input_items,
