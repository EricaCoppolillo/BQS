...
The dataset is a json with the form

    Dictionary structured as follows:
    {
         'users' : #users,
         'popularity' : dict(item: #users),
         'thresholds' : [low, mid],
         'prefs': dict(user: (list_positives, list_negatives_sampled))
    }
    
    train, validation_tr, test_tr hanno il seguente formato: 
    - list_positives: sono tutti i positivi dell'utente (meno quelli che vanno in te)
    - list_negatives_sampled: sono 100 elementi negativi campionati tra tutti i negativi dell'utente
    
    validation_te, test_te hanno il seguente formato:
    - list_positives: sono (al più) tre elementi campionati da low_pop, mid_pop, top_pop
    - list_negatives_sampled: sono 100 elementi negativi campionati tra tutti i negativi dell'utente
...
The whole dataset will be dumped to a dictionary structured as follows:
...
`{
     ...
     'popularity' : dict(item: #users),
     'thresholds' : [low, mid],
     'prefs': dict(user: dict('train': positives_train||
                              'vad_tr': positives_vd_tr,
                              'vad_te': positives_vd_te||
                              'test_tr': positives_test_tr,
                              'test_te': positives_test_te))
}`

- positives_train, positives_vd_tr, positives_test_tr are lists of elements
- positives_vd_te and positives_test_te are lists with at most 3 elements (selected by popularity)

Each user has only the following active keys:
- 'train', 
- 'vad_tr' and 'vad_te'
- 'test_tr' and 'test_te'
That is, a user belongs either to train, validation or test set
...
1: train:{1,12,10,4}
2: train:{1,4,3,8,7}
3: vad_tr: {3,5}, vad_te:{6,2,1}
4: test_tr:{4,15}, test_te:{8,12,10}
...
1: train:{1,12,10,4}
2: train:{1,4,3,8,7}
3: train: {3,5}, vad_te:{6,2,1}
4: train:{4,15}, test_te:{8,12,10}
"""
...
class DataLoader:
    def __init__(self, file_tr, pos_neg_ratio=2, negatives_in_test=100, pos_pos_pairs=False):
    ...
    def _closest_more_popular_item(self, item, positive_items):
        # objects more popular than 'item'
        more_popular_positive_items = np.array(list(filter(lambda x: 
                                                           self.item_popularity_np[x] > self.item_popularity_np[item], 
                                                           positive_items)), dtype=np.uint32)
        if len(more_popular_positive_items)==0:
          return None

        # ordering more_popular_positive_items by popularity (ASC)
        more_popular_positive_items_popularity = self.item_popularity_np[more_popular_positive_items]
        more_popular_positive_items = more_popular_positive_items[np.argsort(more_popular_positive_items_popularity)]

        return more_popular_positive_items[0]
    
    def _less_popular_items(self, pos):
        max_idx = None
        max_item = None
        max_pop = None
        
        for idx in range(len(pos)):
          if max_idx is None or self.item_popularity[pos[idx]]>max_pop:
            max_idx = idx
            max_item = pos[idx]
            max_pop = self.item_popularity[pos[idx]]
        
        filtered_pos = pos[:]
        filtered_pos.remove(max_item)

        """
        print('len(pos):',len(pos))
        print('len(filtered_pos):',len(filtered_pos))
        print('removed item:', max_item)
        print('popularity of the removed item:', max_pop)
        """

        return filtered_pos

    def _generate_training_pairs(self, pos):
        if not self.pos_pos_pairs:
          return self._generate_pairs(pos)
        else:
          positives_a = (pos * self.pos_neg_ratio)
          positives_b = self._less_popular_items(pos)
          positives = positives_a + positives_b
          #random.shuffle(positives)
          negatives = self._sample_negatives(pos, len(positives_a)) + [self._closest_more_popular_item(item, pos) for item in positives_b]
          
          """
          if len(positives)<10:

            print('LC > Pos:',pos)
            print('LC > positives_a:',positives_a)
...
class DataLoader:
    def __init__(self, file_tr, pos_neg_ratio=10, negatives_in_test=100, pos_pos_pairs=False):
    ...
    def _load_data(self, dataset):
        ...
        self.size['train'] = len(train)
        if len(validation) > 0:
            self.data['validatiolog_p(n'] = np.array(validation, dtype=np.int8)
            self.size['validation'] = len(validation)
        if len(test) > 0:
            self.data['test'] = np.array(test, dtype=np.int8)
            self.size['test'] = len(test)
...
class rvae_rank_pair_loss(rvae_loss):
    ...
    def log_p(self, x, y, pos_items, neg_items, mask):
        ...
        if ones>100 and False:
          print('LC> non_zero(mask[0,:]):',ones)
          #print('LC> mask[0,:]:',mask[0,:])
          print('LC> non_zero(pos_items[0,:]):',(pos_items.long()[0,:] != 0).sum(dim=0).item())
          print('LC> pos_items[0,:]:',pos_items.long()[0,:])
          print('LC> non_zero(neg_items[0,:]):',(neg_items.long()[0,:] != 0).sum(dim=0).item())
          print('LC> neg_items[0,:]:',neg_items.long()[0,:])
         
          print('POSITIVE ----------')
          #print('LC> torch.gather(y, 1, (pos_items).long())[0,:]:',torch.gather(y, 1, (pos_items).long())[0,:])
          print('LC> non_zero(torch.gather(y, 1, (pos_items).long())[0,:]*mask[0,:]):',(torch.gather(y, 1, (pos_items).long())[0,:]*mask[0,:] != 0).sum(dim=0).item())
          print('LC> torch.gather(y, 1, (pos_items).long())[0,:]*mask[0,:]:',torch.gather(y, 1, (pos_items).long())[0,:]*mask[0,:])
          
          print('NEGATIVE ----------')
          #print('LC> torch.gather(y, 1, (neg_items).long())[0,:]:',torch.gather(y, 1, (neg_items).long())[0,:])
          print('LC> non_zero(torch.gather(y, 1, (neg_items).long())[0,:]*mask[0,:]):',(torch.gather(y, 1, (neg_items).long())[0,:]*mask[0,:] != 0).sum(dim=0).item())
          print('LC> torch.gather(y, 1, (neg_items).long())[0,:]*mask[0,:]:',torch.gather(y, 1, (neg_items).long())[0,:]*mask[0,:])
 
        #print('LC> torch.gather(y, 1, (neg_items).long()):',torch.gather(y, 1, (neg_items).long()))
        ...
        exp_pop_pos = torch.pow(10,(1-pop_pos))
        exp_pop_neg = torch.pow(10,(1-pop_neg))

        k_pop_pos = 1/(1-pop_pos+eps)-1/(pop_pos+eps)
        
        
        if False:
          print('-'*80)
          print('y1[0]:',y1[0,:10])
          print('non_zero(y1[0]):',(y1[0,:] != 0).sum(dim=0).item())
          print('y2[0]:',y2[0,:10])
          print('non_zero(y2[0]):',(y2[0,:] != 0).sum(dim=0).item())
          #print('type(weight):',type(weight))
          #print('weight.size():',weight.size())
          #print('weight[0]:',weight[0])
          print('non_zero(weight[0]):',(weight[0] != 0).sum(dim=0).item())
          #print('pop_pos[0]:',pop_pos[0])
          print('-'*80)
        ...
        p = 1000 - shift
        l = self.thresholds[1]
        g = p * (torch.tanh((l-pop_pos) * r) + (p + shift) / p)
        ...
        #neg_ll = - torch.sum(self.logsigmoid((exp_pop_pos*y1 - y2))*weight) / mask.sum() 
        #neg_ll = - torch.sum(self.logsigmoid((torch.exp(y1) - torch.exp(y2)))*weight) / mask.sum()
        #neg_ll = - torch.sum(self.logsigmoid((y1 - y2)/exp_pop_pos)*weight) / mask.sum()
        
        #neg_ll = - torch.sum(self.logsigmoid((y1 - y2)/(pop_pos+0.000000001))*weight) / mask.sum()
        #neg_ll= -torch.sum(torch.log(y1-y2)*weight) / mask.sum() 

        neg_ll = -torch.sum(self.logsigmoid((y1 - y2))*weight) / mask.sum()
        
        #for i in range(10):
          #print('(pop,g,l,tresholds):',pop_pos[0,i].item(),g[0,i].item(),pop_pos[0,i].item()<=l,self.thresholds[0],self.thresholds[1])
