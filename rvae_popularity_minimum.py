...
class rvae_rank_pair_loss(rvae_loss):
    ...
    def log_p(self, x, y, pos_items, neg_items, mask):
        ...
        # print('type(pos_items):',type(pos_items))
        # print('type(pos_items._values()):', type(pos_items._values()))
        # print('pos_items._values():', pos_items._values())
        # print('y.shape:', y.shape)
        # print('pos_items._values().long().shape:', pos_items._values().long().shape)
        # print('pos_items.long().shape:', pos_items.long().shape)

        # y1 = torch.gather(y, 1, pos_items._values().long(), sparse_grad=False) * mask
        # y2 = torch.gather(y, 1, neg_items._values().long(), sparse_grad=False) * mask

        # _mask = mask.to_dense()
        # _pos_items = pos_items.long().to_dense()
        # _neg_items = neg_items.long().to_dense()

        _mask = mask
        _pos_items = pos_items.long()
        _neg_items = neg_items.long()

        y1 = torch.gather(y, 1, _pos_items, sparse_grad=False) * _mask
        y2 = torch.gather(y, 1, _neg_items, sparse_grad=False) * _mask

        neg_ll = -torch.sum(self.logsigmoid(y1 - y2) * _mask) / _mask.sum()
