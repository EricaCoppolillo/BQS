...
class DataLoader:
    ...
    def _load_data(self, dataset):
        ...
        for user_id in training_data:
            ...
            self.number_of_items.append(len(pos))
    ...
    def iter(self, batch_size=256, tag='train'):
        ...
        for start_idx in range(0, N, batch_size):
            ...
            # mask = np.zeros(shape=(number_of_rows, self.max_width))
            mask = lil_matrix((number_of_rows, self.max_width))
            for row in range(number_of_rows):
                # print('start_idx+row:', start_idx + row)
                # print('self.number_of_items[start_idx + row]:', self.number_of_items[start_idx + row])
                # print('self.number_of_items[start_idx+row]:', self.number_of_items[start_idx + row])
                mask[row, :self.number_of_items[start_idx + row]] = [1] * self.number_of_items[start_idx + row]
...
class rvae_rank_pair_loss(rvae_loss):
    ...
    def log_p(self, x, y, pos_items, neg_items, mask):
        ...
        neg_ll = -torch.sum(self.logsigmoid(y1 - y2) * mask) / mask.sum()
