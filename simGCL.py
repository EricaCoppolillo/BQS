...
from simgcl_models import SimGCL
from simgcl_loss import cal_cl_loss, l2_reg_loss
from simgcl_utils import create_sparse_bipartite_adjacency, normalize_graph_mat
...
result_dir = os.path.join(data_dir, 'simGCL', 'results')
...
# --- simGCL preprocessing ---
dataset_var = load_dataset(dataset_file)
simgcl_dataset = []
for user_idx in dataset_var['training_data']:
    for item_idx in dataset_var['training_data'][user_idx][0]:
        simgcl_dataset.append([user_idx, item_idx])
adj_mat = create_sparse_bipartite_adjacency(simgcl_dataset,
                                            dataset_var['users'],
                                            dataset_var['items'])
adj_mat = normalize_graph_mat(adj_mat)
del dataset_var
# --- simGCL preprocessing ---
...
def train(dataloader, epoch, optimizer): # embeddings_dict
    ...
    for batch_idx, (triplets, mask) in enumerate(dataloader.iter(batch_size=config.batch_size)):
        users = torch.tensor(triplets[:, 0], device=device).long()
        pos = torch.tensor(triplets[:, 1], device=device).long()
        neg = torch.tensor(triplets[:, 2], device=device).long()
        ...
        # TRAIN on batch
        optimizer.zero_grad()
        y, user_embed, pos_item_embed = model.predict(users, pos, neg)

        if config.regularizer == "PD":
            y = elu_func(y) + 1
            y = torch.einsum('b,b -> b', y, torch_pop[pos])

        ranking_loss = criterion(y, mask)
        contrastive_loss = cal_cl_loss(model, [users, pos])*config.lambda_val
        l2_loss = l2_reg_loss(config.l2_coeff, user_embed, pos_item_embed)

        if config.regularizer == "boratto":
            # computing the absolute correlation
            vx = ranking_loss - torch.mean(ranking_loss)
            ...
            ranking_loss = torch.sum(ranking_loss)
            ranking_loss += correlation * reg_weight
        else:
            ranking_loss = torch.sum(ranking_loss)

        loss = ranking_loss + contrastive_loss + l2_loss
...
model = SimGCL(norm_adj=adj_mat,
                 emb_size=config.latent_dim,
                 eps=config.epsilon,
...
if copy_pasting_data:
    main_directory = os.path.join('./data', dataset_name, "simGCL", model_type)
