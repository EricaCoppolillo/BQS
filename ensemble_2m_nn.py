...
class EnsembleMultiVAE(nn.Module):
    ...
    def __init__(self, popularity, thresholds):
        ...
        self.bn_a_2 = torch.nn.BatchNorm1d(n_items)
        self.bn_b_2 = torch.nn.BatchNorm1d(n_items)

        self.bn_e_1 = torch.nn.BatchNorm1d(n_items)
        self.bn_e_2 = torch.nn.BatchNorm1d(n_items)

        self.d_a_1 = torch.nn.Dropout(0.2)
        self.d_b_1 = torch.nn.Dropout(0.2)

        self.d_a_2 = torch.nn.Dropout(0.2)
        self.d_b_2 = torch.nn.Dropout(0.2)

        self.d_e_1 = torch.nn.Dropout(0.2)
        self.d_e_2 = torch.nn.Dropout(0.2)

        self.relu_a_1 = torch.nn.ReLU()
        self.relu_b_1 = torch.nn.ReLU()

        self.relu_e_1 = torch.nn.ReLU()

        self.tanh_a = torch.nn.Tanh()
        self.tanh_b = torch.nn.Tanh()

        self.tanh_e = torch.nn.Tanh()

        self.bn = torch.nn.BatchNorm1d(n_items)
        self.bn2 = torch.nn.BatchNorm1d(2 * n_items)
    ...
    def forward_2(self, x, popularity, y_a, y_b, predict=False):
        ...
        # z_a = torch.softmax(y_a, self.n_items)
        # z_b = torch.softmax(y_b, self.n_items)
        ...
        # z_a = self.dropout(z_a)
        # z_b = self.dropout(z_b)
